# HANDOFF – VentRunCheck (dryer vent cleaning)

**Date**: 2026-09-04 (HK local / UTC+8)

## Build

- Command: `npm run build`
- Result: **success** — exit code `0`
- Output: `/workspace/mogul/sites/dryer-vent-cleaning/dist/`
- Pages built: 7

## First Guide draft

The DIY vs Pro guide **is** the first Guide draft.

- Source: `/workspace/mogul/sites/dryer-vent-cleaning/src/content/guides/diy-vs-pro-dryer-vent-cleaning.mdx`
- Built URL path: `/guides/diy-vs-pro-dryer-vent-cleaning/`
- Dist file: `dist/guides/diy-vs-pro-dryer-vent-cleaning/index.html`

## Three example pages

| Template  | Source  | Dist |
| --- | --- | --- |
| Guide | `src/content/guides/diy-vs-pro-dryer-vent-cleaning.mdx` | `dist/guides/diy-vs-pro-dryer-vent-cleaning/index.html` |
| Compare | `src/content/compare/brush-kit-vs-leaf-blower.mdx` | `dist/compare/brush-kit-vs-leaf-blower/index.html` |
| Checklist | `src/content/checklist/before-you-run-the-dryer-again.mdx` | `dist/checklist/before-you-run-the-dryer-again/index.html` |

## Caveats

- Brand **VentRunCheck** is **locked** (Ronan approved 2026-09-04).
- Affiliate links use `?ref=PENDING` only — no real IDs.
- No deploy, domain, outreach mail, or real analytics added.
- npm engine warning: local npm is 9.2.0; Astro 4.x prefers >=9.6.5 — build still exited 0.
- Guide/Compare/Checklist layouts use a default slot (section order lives in MDX) plus fixed affiliate blurb on Guide/Compare.
- Stack pinned to Astro 4.x + MDX 3.x for Node 20 compatibility (latest stable line that builds cleanly here).

## Status

- Income: **0**
- Not deployed

## Nightly 2026-09-05

- New Guide: src/content/guides/how-often-clean-dryer-vent.mdx
  Built: /guides/how-often-clean-dryer-vent/ (dist/guides/how-often-clean-dryer-vent/index.html)
- Build exit code: 0 (8 pages)
- Income: **0**
- Not deployed

## Nightly 2026-09-07

- New Compare: src/content/compare/metal-transition-vs-foil-duct.mdx
  Built: /compare/metal-transition-vs-foil-duct/ (dist/compare/metal-transition-vs-foil-duct/index.html)
- Build exit code: 0 (9 pages)
- Income: **0**
- Not deployed

## Nightly 2026-09-08

- New Checklist: src/content/checklist/outdoor-vent-cap-inspection.mdx
  Built: /checklist/outdoor-vent-cap-inspection/ (dist/checklist/outdoor-vent-cap-inspection/index.html)
- Build exit code: 0 (10 pages)
- Income: **0**
- Not deployed

## Nightly 2026-09-09

- New Guide: src/content/guides/signs-clogged-dryer-vent.mdx
  Built: /guides/signs-clogged-dryer-vent/ (dist/guides/signs-clogged-dryer-vent/index.html)
- Build exit code: 0 (11 pages)
- Income: **0**
- Not deployed

## Nightly 2026-09-10

- New Compare: src/content/compare/booster-fan-vs-cleaning-clog.mdx
  Built: /compare/booster-fan-vs-cleaning-clog/ (dist/compare/booster-fan-vs-cleaning-clog/index.html)
- Build exit code: 0 (12 pages)
- Income: **0**
- Not deployed

## Nightly 2026-09-14

- New Checklist: src/content/checklist/behind-the-dryer-inspection.mdx
  Built: /checklist/behind-the-dryer-inspection/ (dist/checklist/behind-the-dryer-inspection/index.html)
- Build exit code: 0 (13 pages)
- Pushed: origin main `5e1d919` (GitHub Pages via Actions)
- Live URL (after Pages): https://rcloudso.github.io/ventruncheck/checklist/behind-the-dryer-inspection/
- Income: **0**

## Nightly 2026-09-15

- New Guide: src/content/guides/dont-replace-dryer-check-vent-first.mdx
  Built: /guides/dont-replace-dryer-check-vent-first/ (dist/guides/dont-replace-dryer-check-vent-first/index.html)
- Build exit code: 0 (14 pages)
- Pushed: origin main `e395230` (GitHub Pages via Actions)
- Live URL (after Pages): https://rcloudso.github.io/ventruncheck/guides/dont-replace-dryer-check-vent-first/
- Income: **0**

## Nightly 2026-09-16

- New Compare: src/content/compare/lint-screen-vs-full-vent-cleaning.mdx
  Built: /compare/lint-screen-vs-full-vent-cleaning/ (dist/compare/lint-screen-vs-full-vent-cleaning/index.html)
- Build exit code: 0 (15 pages)
- Pushed: origin main `2954b04` (GitHub Pages via Actions)
- Live URL (after Pages): https://rcloudso.github.io/ventruncheck/compare/lint-screen-vs-full-vent-cleaning/
- Income: **0**

## Nightly 2026-09-17

- New Checklist: src/content/checklist/slow-drying-clothes-checklist.mdx
  Built: /checklist/slow-drying-clothes-checklist/ (dist/checklist/slow-drying-clothes-checklist/index.html)
- Build exit code: 0 (16 pages)
- Pushed: origin main `8607a13` (GitHub Pages via Actions)
- Live URL (after Pages): https://rcloudso.github.io/ventruncheck/checklist/slow-drying-clothes-checklist/
- Income: **0**

## Nightly 2026-09-18

- New Guide: src/content/guides/burning-smell-from-dryer.mdx
  Built: /guides/burning-smell-from-dryer/ (dist/guides/burning-smell-from-dryer/index.html)
- Build exit code: 0 (17 pages)
- Pushed: origin main `673c67a` (GitHub Pages via Actions)
- Live URL (after Pages): https://rcloudso.github.io/ventruncheck/guides/burning-smell-from-dryer/
- Income: **0**

## Nightly 2026-09-21

- New Compare: src/content/compare/wall-vs-roof-dryer-vent-termination.mdx
  Built: /compare/wall-vs-roof-dryer-vent-termination/ (dist/compare/wall-vs-roof-dryer-vent-termination/index.html)
- Build exit code: 0 (18 pages)
- Pushed: origin main `4ed28e9` (GitHub Pages via Actions)
- Live URL (after Pages): https://rcloudso.github.io/ventruncheck/compare/wall-vs-roof-dryer-vent-termination/
- Income: **0**

## Nightly 2026-09-22

- New Checklist: src/content/checklist/dryer-vent-cleaning-prep.mdx
  Built: /checklist/dryer-vent-cleaning-prep/ (dist/checklist/dryer-vent-cleaning-prep/index.html)
- Build exit code: 0 (19 pages)
- Pushed: origin main `2b9d613` (GitHub Pages via Actions)
- Live URL (after Pages): https://rcloudso.github.io/ventruncheck/checklist/dryer-vent-cleaning-prep/
- Income: **0**

## Nightly 2026-09-23

- New Guide: src/content/guides/when-dryer-vent-run-is-too-long.mdx
  Built: /guides/when-dryer-vent-run-is-too-long/ (dist/guides/when-dryer-vent-run-is-too-long/index.html)
- Build exit code: 0 (20 pages)
- Pushed: origin main `d1297c2` (GitHub Pages via Actions)
- Live URL (after Pages): https://rcloudso.github.io/ventruncheck/guides/when-dryer-vent-run-is-too-long/
- Income: **0**
